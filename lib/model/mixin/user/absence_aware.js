...
module.exports = function(sequelize){
  ...
  this._get_calendar_months_to_show = function(args){
    ...
    if (show_full_year) {
      return _.map(orderMonths, function(i){
         ...
         var mon = String(i).padStart(2, '0');
        return moment.utc(holYear.format('YYYY')+'-'+mon+'-01');
      });
    }
  ...
  this.promise_calendar = function(args) {
    var
     ...
     holiday_year_start_month           = args.holiday_year_start_month  || 0,
    ...
    var months_to_show = this_user._get_calendar_months_to_show({
      ...
      starting_month :holiday_year_start_month
    });
        ...
        return this_user.getMy_leaves({
          where : {
            ...
            $or : {
              date_start : {
                $between : [
                  moment.utc(year).subtract(1, "year").month(args.holiday_year_start_month ).startOf('month').format('YYYY-MM-DD'),
                  moment.utc(year).month(args.holiday_year_start_month -1).endOf('month').format('YYYY-MM-DD HH:mm'),
                ]
              },
              date_end : {
                $between : [
  ...
  this.promise_my_active_leaves = function(args) {
    ...
    var holiday_year_start_month  = args.holiday_year_start_month ;
  ...
  this.promise_my_active_leaves_ever = function() {
    ...
    return this.promise_my_leaves({
      ignore_year : true,
      holiday_year_start_month: this.company.holiday_year_start_month,
      filter_status : [
        sequelize.models.Leave.status_approved(),
        sequelize.models.Leave.status_new(),
        sequelize.models.Leave.status_pended_revoke(),
  ...
  this.promise_adjustment_for_year = function(holiday_year_start_month ,year){
    let self = this;

    return self
      .promise_adjustment_and_carry_over_for_year(holiday_year_start_month ,year)
      .then(combined_record => Promise.resolve(combined_record.adjustment));
  };

  this.promise_carried_over_allowance_for_year = function(holiday_year_start_month ,year){
    let self = this;
      ...
      where : {
        ...
        $or : {
          date_start : {
            $between : [
              moment.utc(year).subtract(1, "year").month(args.holiday_year_start_month ).startOf('month').format('YYYY-MM-DD'),
              moment.utc(year).month(args.holiday_year_start_month ).endOf('month').format('YYYY-MM-DD HH:mm'),
            ]
          },
          date_end : {
            $between : [
  ...
  this.validate_leave_fits_into_remaining_allowance = function(args){
    ...
    .then(employee =>
      employee.promise_allowance({year,holiday_year_start_month })
    ...
    .then(function(employee){
      if (
        ...
        // ... sum of used days for this limit is going to be bigger then limit
